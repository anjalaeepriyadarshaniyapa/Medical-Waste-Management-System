    ...
    void initialize() {
        ...
        deleteButton.setOnAction(event -> {
            if (!DeleteID_txt.getText().trim().equals("")) {
                databaseHandler.deleteRecordFromWasteTable(Const.INFECTIOUS_WASTE_TABLE, Integer.parseInt(DeleteID_txt.getText().trim()));
        ...
        updateButton.setOnAction(event -> {
            if (!UpdateID_txt.getText().trim().equals("")) {
                ...
                try {
                    databaseHandler.updateRecordOfWasteTable(Const.INFECTIOUS_WASTE_TABLE, id, infectious_waste);
                    Alert alert = new Alert(AlertType.INFORMATION);
                    alert.setTitle("Success");
        ...
        loadButton.setOnAction(event -> {
            ...
            ResultSet resultSet = databaseHandler.populateTableData(Const.INFECTIOUS_WASTE_TABLE);
        ...
        savePDFButton.setOnAction(event -> {
            String filename = "GeneratedPDF/Infectious_Waste_Details.pdf"; //PDF will be saved to the GeneratedPDF folder in project directory root
            PDFSaver pdfSaver = new PDFSaver();
            pdfSaver.SaveFile(Const.INFECTIOUS_WASTE_TABLE, "Infectious Waste Details", filename, databaseHandler);
        });
        ...
        insertDataButton.setOnAction(event -> {
            if (!typeOfWaste_txt.getText().trim().equals("") ||
                ...
                infectious_waste.setType_of_container(typeOfContainer_txt.getText().trim());
                infectious_waste.setWasteperday(Float.parseFloat(totalWastePerDay_txt.getText().trim()));
                databaseHandler.insertWasteData(Const.INFECTIOUS_WASTE_TABLE, infectious_waste);
                Alert alert = new Alert(AlertType.INFORMATION);
                alert.setTitle("Success");
